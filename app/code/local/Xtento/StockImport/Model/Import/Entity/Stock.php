 ...
 * Last Modified: 2014-04-08T14:34:08+02:00
...
class Xtento_StockImport_Model_Import_Entity_Stock extends Xtento_StockImport_Model_Import_Entity_Abstract
{
    ...
    public function prepareImport($updatesInFilesToProcess)
    {
        ...
        if (!in_array('qty', $fieldsFound) && !in_array('is_in_stock', $fieldsFound) && !in_array('manage_stock', $fieldsFound)) {
    ...
    private function _getProductIdsForProductIdentifiers()
    {
        ...
        if ($this->_attributeToLoadBy == 'sku') {
            ...
            foreach ($this->_productIdentifiers as $productIdentifier) {
                $this->_productMap[$productIdentifier] = $productIdentifier;
            }
        }
    }
    ...
    private function _getCurrentStockInfo()
    {
        ...
        $select = $this->_readAdapter->select()
            ->from($this->getTableName('cataloginventory/stock_item'), array('product_id', 'qty', 'is_in_stock', 'stock_id', 'manage_stock', 'use_config_manage_stock', 'min_qty', 'use_config_min_qty'))
    ...
    private function _getCurrentProductInfo()
    {
        if (self::$importProductStatus) {
            ...
            if ($attributeId) {
                ...
                foreach ($products as $product) {
                    $this->_productStatus[$product['entity_id']] = array('status' => (bool)$product['value']);
                }
            }
        }
    }
                    ...
                    if (isset($this->_productStatus[$productId])) {
                        ...
                        if ($updateData['status'] != '') {
                            ...
                            if ($newStatus === null && $updateStatus !== '') {
                                Mage::throwException('An invalid value was imported for the product status column. It should contain values as "Enabled" or "Disabled".');
                            }
                ...
                if ($field == $stockField) {
                    ...
                    if ($stockField == 'qty') {
                        ...
                        if (!is_numeric($newValue)) {
                            ...
                            if ($newValue === 'yes' || $newValue === 'ja' || $newValue === 'in stock' || $newValue === 'available') {
                                $newValue = 5000;
                            }
                            if ($newValue === 'no' || $newValue === 'nein' || $newValue === 'out of stock' || $newValue === 'oos') {
                                $newValue = 0;
                            }
                        }
                        #$stockValue = (float)preg_replace("/[^0-9.,+-]/", "", $stockValue); // Float value coming from the database
                    }
                    if ($stockField == 'manage_stock' || $stockField == 'is_in_stock') {
            ...
            if (self::$importPrices || self::$importSpecialPrices) {
                if (Mage::helper('xtcore/utils')->mageVersionCompare(Mage::getVersion(), '1.4.0.0', '>=')) {
                    ...
                    foreach ($processes as $indexerProcess) {
                        ...
                        if (!empty($this->_updatedPrices) || !empty($this->_updatedSpecialPrices)) {
